...
"Z":"")}JSCompiler_temp=value}else JSCompiler_temp=reFormat.test(value+="")?'"'+value.replace(/"/g,'""')+'"':value;value=JSCompiler_temp}return value}var reFormat=new RegExp('["'+delimiter+"\n\r]"),DELIMITER=delimiter.charCodeAt(0);return{parse:function(text,f){var convert,columns;text=parseRows(text,function(row,i){if(convert)return convert(row,i-1);columns=row;convert=f?customConverter(row,f):objectConverter(row)});text.columns=columns||[];return text},parseRows,format:function(rows,columns){null==
columns&&(columns=inferColumns(rows));return[columns.map(formatValue).join(delimiter)].concat(preformatBody(rows,columns)).join("\n")},formatBody:function(rows,columns){null==columns&&(columns=inferColumns(rows));return preformatBody(rows,columns).join("\n")},formatRows:function(rows){return rows.map(formatRow).join("\n")},formatRow,formatValue}};var EOL={},EOF={},QUOTE=34,NEWLINE=10,RETURN=13}
