...
for(i=0;i<n;++i)if(gesture=gestures[touches[i].identifier])(0,_noevent.default)(event),gesture("drag",event,touches[i])}function touchended(event){var touches=event.changedTouches,n=touches.length,i,gesture;touchending&&clearTimeout(touchending);touchending=setTimeout(function(){touchending=null},500);for(i=0;i<n;++i)if(gesture=gestures[touches[i].identifier])(0,_noevent.nopropagation)(event),gesture("end",event,touches[i])}function beforestart(that,container,event$jscomp$0,d,identifier,touch){var dispatch=
listeners.copy(),p=(0,_d3Selection.pointer)(touch||event$jscomp$0,container),s;if(null!=(s=subject.call(that,new _event.default("beforestart",{sourceEvent:event$jscomp$0,target:drag,identifier,active,x:p[0],y:p[1],dx:0,dy:0,dispatch}),d))){var dx=s.x-p[0]||0;var dy=s.y-p[1]||0;return function gesture(type,event,touch){var p0=p;switch(type){case "start":gestures[identifier]=gesture;var n=active++;break;case "end":delete gestures[identifier],--active;case "drag":p=(0,_d3Selection.pointer)(touch||event,
container),n=active}dispatch.call(type,that,new _event.default(type,{sourceEvent:event,subject:s,target:drag,identifier,active:n,x:p[0]+dx,y:p[1]+dy,dx:p[0]-p0[0],dy:p[1]-p0[1],dispatch}),d)}}}var filter=defaultFilter,container=defaultContainer,subject=defaultSubject,touchable=defaultTouchable,gestures={},listeners=(0,_d3Dispatch.dispatch)("start","drag","end"),active=0,mousedownx,mousedowny,mousemoving,touchending,clickDistance2=0;drag.filter=function(_){return arguments.length?(filter="function"===
typeof _?_:(0,_constant.default)(!!_),drag):filter};drag.container=function(_){return arguments.length?(container="function"===typeof _?_:(0,_constant.default)(_),drag):container};drag.subject=function(_){return arguments.length?(subject="function"===typeof _?_:(0,_constant.default)(_),drag):subject};drag.touchable=function(_){return arguments.length?(touchable="function"===typeof _?_:(0,_constant.default)(!!_),drag):touchable};drag.on=function(){var value=listeners.on.apply(listeners,arguments);
