...
i,data)+o)*pow2k,wi=+weight(d,i,data);if(wi&&0<=xi&&xi<n&&0<=yi&&yi<m){var x0=Math.floor(xi),y0=Math.floor(yi);xi=xi-x0-.5;yi=yi-y0-.5;values[x0+y0*n]+=(1-xi)*(1-yi)*wi;values[x0+1+y0*n]+=xi*(1-yi)*wi;values[x0+1+(y0+1)*n]+=xi*yi*wi;values[x0+(y0+1)*n]+=(1-xi)*yi*wi}}(0,_d3Array.blur2)({data:values,width:n,height:m},r*pow2k);return values}function density(data){data=grid(data);var tz=threshold(data),pow4k=Math.pow(2,2*k);Array.isArray(tz)||(tz=(0,_d3Array.ticks)(Number.MIN_VALUE,(0,_d3Array.max)(data)/
