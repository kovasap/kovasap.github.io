...
coordinates:input.coordinates.map(quantizePoint)};break;default:return input}null!=input.id&&(output.id=input.id);null!=input.bbox&&(output.bbox=input.bbox);null!=input.properties&&(output.properties=input.properties);return output}if(topology.transform)throw Error("already quantized");if(transform&&transform.scale)var box=topology.bbox;else{if(!(2<=(n$jscomp$0=Math.floor(transform))))throw Error("n must be ≥2");box=topology.bbox||bbox(topology);transform=box[0];var y0=box[1],x1=box[2],y1=box[3],
n$jscomp$0;transform={scale:[x1-transform?(x1-transform)/(n$jscomp$0-1):1,y1-y0?(y1-y0)/(n$jscomp$0-1):1],translate:[transform,y0]}}var t=untransform(transform),key;y0=topology.objects;x1={};for(key in y0)x1[key]=quantizeGeometry(y0[key]);return{type:"Topology",bbox:box,transform,objects:x1,arcs:topology.arcs.map(function(input){var i=0,j=1,n=input.length,p,output=Array(n);for(output[0]=t(input[0],0);++i<n;)if((p=t(input[i],i))[0]||p[1])output[j++]=p;1===j&&(output[j++]=[0,0]);output.length=j;return output})}};
exports.transform=transform$jscomp$0;exports.untransform=untransform;Object.defineProperty(exports,"__esModule",{value:!0})})}
